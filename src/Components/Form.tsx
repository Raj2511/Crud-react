...
const Form: FC = () => {
  ...
  const [submittedValues, setSubmittedName] = useState<{name: string; mail: string, city: string, status: boolean} | null>(null);
  ...
  const handleSubmit = () => {
    if (namefield.trim() !== '' && mailfield.trim() !== '' && cityfield.trim() !== '') {
      setSubmittedName({name: namefield, mail: mailfield, city: cityfield, status: statusfield});
